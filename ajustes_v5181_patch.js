...
function afterSaveChamado(o){
  ...
  const venda=window.lcCriarVendaDoChamado(o);
  if(venda) abrirVendaFaturada(venda);
}
...
// 1.2.2 excluir venda ligada a chamado → apaga o chamado
const _exc=window.excluirVendaUnificado;
if(typeof _exc==='function' && !_exc.__v5181){
  window.excluirVendaUnificado=function(){
    ...
    let alvos=[];
    if(checks.length) alvos=checks.map(ch=>(db.vendas||[]).find(x=>x.id===ch.value)).filter(Boolean);
    else {
      const vId=window.neoVendaSelecionada||window.vendaSelecionadaId;
      const v=vId&&(db.vendas||[]).find(x=>x.id===vId); if(v) alvos=[v];
    }
    const ligadas=alvos.filter(v=>v.chamadoId);
    if(!ligadas.length) return _exc.apply(this,arguments);
    const msg='Deseja apagar '+alvos.length+' venda(s)?\n\nEntre as vendas, '+ligadas.length+' '+(ligadas.length===1?'é relacionada':'são relacionadas')+' a chamados. Apagá-la(s) irá apagar o chamado junto.';
    confirmar(msg,'Excluir vendas de chamado').then(ok=>{
      if(!ok) return;
      const fat=alvos.filter(v=>['faturado','finalizada','concluido','pago'].includes(String(v.status||'').toLowerCase()));
      if(fat.length){ aviso('Não é possível excluir vendas faturadas. Estorne primeiro.','Excluir'); return; }
      alvos.forEach(v=>{
        (v.itens||[]).forEach(it=>{
          const p=(db.produtos||[]).find(x=>x.id===it.produtoId);
          if(p&&p.categoria!=='Serviço'&&p.categoria!=='Recarga') p.estoque=n(p.estoque)+n(it.qtd,1);
        });
        if(v.chamadoId) db.os=(db.os||[]).filter(o=>o.id!==v.chamadoId);
      ...
      aviso(alvos.length+' venda(s) excluída(s). Chamado(s) relacionado(s) apagado(s).','Excluído');
    });
  };
  window.excluirVendaUnificado.__v5181=true;
}
