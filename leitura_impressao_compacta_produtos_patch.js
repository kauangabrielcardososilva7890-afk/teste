...
function empAtual(){ const s=typeof getSession==='function'?getSession():null; return (db.empresas||[]).find(e=>e.id===(s&&s.empresaId)) || (db.empresas||[])[0] || {}; }
...
function htmlNotinhaLeitura(leituraId, departamento='todos'){
  ...
  <div class="top"><img src="./logo.png" class="logo"><div class="empresa"><h1>${esc(emp.nome||emp.fantasia||'DIGICOPY')}</h1><div>${esc(emp.cnpj||'')} ${emp.fone||emp.telefone?'• Tel. '+esc(emp.fone||emp.telefone):''}</div><div>${esc([emp.endereco,emp.numero,emp.bairro,emp.cidade||emp.municipio,emp.uf||emp.estado].filter(Boolean).join(' - '))}</div></div><div><b>Leitura:</b> ${esc(l.numero||'')}<br><b>Emissão:</b> ${dataBR(new Date())}</div></div>
