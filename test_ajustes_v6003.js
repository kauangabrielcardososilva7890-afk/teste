...
ok('manifesto já é 229 (v6.1.3 fechava a fila; v7.0.20 soma o mandar-erro, v7.0.22 o portão, v7.0.24 a função única no fim)', manifest.length >= 225 && manifest[manifest.length - 22] === 'autocura_empresa_central_nf_tela_patch.js' && manifest[manifest.length - 21] === 'perfis_nuvem_cura_sessao_patch.js' && manifest[manifest.length - 20] === 'permissoes_estorno_venda_patch.js' && manifest[manifest.length - 19] === 'fiscal_menu_completo_patch.js' && manifest[manifest.length - 18] === 'dashboard_inicio_clicavel_patch.js' && manifest[manifest.length - 17] === 'menus_fiscais_separados_patch.js' && manifest[manifest.length - 16] === 'permissoes_override_menus_fiscais_patch.js' && manifest[manifest.length - 15] === 'seis_submenus_velho_patch.js' && manifest[manifest.length - 14] === 'submenu_hover_nfe_patch.js' && manifest[manifest.length - 13] === 'navegacao_sem_tela_branca_patch.js' && manifest[manifest.length - 12] === 'ribbon_fiscal_estilo_antigo_patch.js' && manifest[manifest.length - 11] === 'fiscal_catalogo_completo_patch.js');
ok('worker atualizado 5.28.0 · gerente segue 5.26.3', fs.readFileSync('cloudflare-worker/src/index.js', 'utf8').indexOf("WORKER_VERSION = '5.28.0'") >= 0 && JSON.parse(fs.readFileSync('gerente-atualizacoes/package.json', 'utf8')).version === '5.26.3');
