...
ok('package.json na 6.0.3', pkg.version === '6.0.11');
ok('index.html carimbado 6.0.3', html.indexOf("DIGICOPY_APP_VERSION = '6.0.11'") >= 0 && html.indexOf('>v6.0.11<') >= 0);
ok('manifesto sobe pra 211 (correção visual vive no patch da central; v6.0.6 fecha a fila)', manifest.length === 216 && manifest[manifest.length - 9] === 'autocura_empresa_central_nf_tela_patch.js' && manifest[manifest.length - 8] === 'perfis_nuvem_cura_sessao_patch.js' && manifest[manifest.length - 7] === 'permissoes_estorno_venda_patch.js' && manifest[manifest.length - 6] === 'fiscal_menu_completo_patch.js' && manifest[manifest.length - 5] === 'dashboard_inicio_clicavel_patch.js' && manifest[manifest.length - 4] === 'menus_fiscais_separados_patch.js' && manifest[manifest.length - 3] === 'permissoes_override_menus_fiscais_patch.js' && manifest[manifest.length - 2] === 'seis_submenus_velho_patch.js' && manifest[manifest.length - 1] === 'submenu_hover_nfe_patch.js');
