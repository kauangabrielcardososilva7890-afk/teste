...
ok('manifesto já é 223 (v6.1.3 fecha a fila com navegação fiscal na barra + escuro íntegro)', manifest.length === 223 && manifest[manifest.length - 16] === 'autocura_empresa_central_nf_tela_patch.js' && manifest[manifest.length - 15] === 'perfis_nuvem_cura_sessao_patch.js' && manifest[manifest.length - 14] === 'permissoes_estorno_venda_patch.js' && manifest[manifest.length - 13] === 'fiscal_menu_completo_patch.js' && manifest[manifest.length - 12] === 'dashboard_inicio_clicavel_patch.js' && manifest[manifest.length - 11] === 'menus_fiscais_separados_patch.js' && manifest[manifest.length - 10] === 'permissoes_override_menus_fiscais_patch.js' && manifest[manifest.length - 9] === 'seis_submenus_velho_patch.js' && manifest[manifest.length - 8] === 'submenu_hover_nfe_patch.js' && manifest[manifest.length - 7] === 'navegacao_sem_tela_branca_patch.js' && manifest[manifest.length - 6] === 'ribbon_fiscal_estilo_antigo_patch.js' && manifest[manifest.length - 5] === 'fiscal_catalogo_completo_patch.js');
ok('worker atualizado 5.26.5 · gerente segue 5.26.3', fs.readFileSync('cloudflare-worker/src/index.js', 'utf8').indexOf("WORKER_VERSION = '5.26.5'") >= 0 && JSON.parse(fs.readFileSync('gerente-atualizacoes/package.json', 'utf8')).version === '5.26.3');
