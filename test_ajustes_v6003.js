...
ok('package.json na 6.0.3', pkg.version === '6.1.2');
ok('index.html carimbado 6.0.3', html.indexOf("DIGICOPY_APP_VERSION = '6.1.2'") >= 0 && html.indexOf('>v6.1.2<') >= 0);
ok('manifesto já é 222 (v6.1.2 fecha a fila com navegação fiscal na barra + escuro íntegro)', manifest.length === 224 && manifest[manifest.length - 17] === 'autocura_empresa_central_nf_tela_patch.js' && manifest[manifest.length - 16] === 'perfis_nuvem_cura_sessao_patch.js' && manifest[manifest.length - 15] === 'permissoes_estorno_venda_patch.js' && manifest[manifest.length - 14] === 'fiscal_menu_completo_patch.js' && manifest[manifest.length - 13] === 'dashboard_inicio_clicavel_patch.js' && manifest[manifest.length - 12] === 'menus_fiscais_separados_patch.js' && manifest[manifest.length - 11] === 'permissoes_override_menus_fiscais_patch.js' && manifest[manifest.length - 10] === 'seis_submenus_velho_patch.js' && manifest[manifest.length - 9] === 'submenu_hover_nfe_patch.js' && manifest[manifest.length - 8] === 'navegacao_sem_tela_branca_patch.js' && manifest[manifest.length - 7] === 'ribbon_fiscal_estilo_antigo_patch.js' && manifest[manifest.length - 6] === 'fiscal_catalogo_completo_patch.js');
