// test_ajustes_v52412.js — v5.24.22: notinha FATURADA → Imprimir inacessível
...
ok(fix.includes('v5.24.22'), 'carimbo v5.24.22 na trava corrigida');
...
ok(bundle.includes('v5.24.22'), 'bundle carrega o carimbo da correção');
...
ok(idx.includes("DIGICOPY_APP_VERSION = '5.24.22'"), 'index: DIGICOPY_APP_VERSION 5.24.22');
ok(idx.includes('>v5.24.22<'), 'index: rodapé v5.24.22');
ok(idx.includes('app.bundle.js?v=5.24.22'), 'index: cache-bust do bundle v5.24.22');
ok(!idx.includes('5.24.11'), 'index: nenhum carimbo velho sobrou');
const mob = fs.readFileSync('mobile/www/index.html', 'utf8');
ok(mob.includes("DIGICOPY_APP_VERSION = '5.24.22'"), 'mobile: DIGICOPY_APP_VERSION 5.24.22');
const worker = fs.readFileSync('cloudflare-worker/src/index.js', 'utf8');
ok(worker.includes("'5.24.22'"), 'worker carimbado 5.24.22');

if (falhas > 0) { console.error(`\n${falhas} assert(s) FALHARAM`); process.exit(1); }
console.log('\nTudo OK — v5.24.22 (Imprimir na faturada destravado).');
