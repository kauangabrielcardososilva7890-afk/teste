...
(function(){
  ...
  window.renderConfig=function(){
    document.getElementById('view-config').innerHTML=neoPage('Configurações', 'Empresa, técnicos e preferências do sistema', `<button onclick="exportBackup()" class="neo-btn"><i class="ph ph-download"></i>Exportar backup</button><button onclick="limparTodosDados()" class="neo-btn danger"><i class="ph ph-trash"></i>Limpar todos os dados</button><button onclick="saveConfig()" class="neo-btn primary"><i class="ph ph-floppy-disk"></i>Salvar</button>`, `<div class="grid grid-cols-1 md:grid-cols-2 gap-3 w-full"><div class="neo-card"><label class="neo-label">Técnico</label><div class="flex gap-2"><input id="new-tecnico-nome" class="neo-input flex-1" placeholder="Nome do técnico"><button onclick="addTecnico()" class="neo-btn primary"><i class="ph ph-plus"></i>Cadastrar</button></div></div><div class="neo-card"><p class="neo-label">Backup e dados</p><p class="text-[12px] text-slate-500 mt-1">Exporte um arquivo JSON de segurança, ou apague tudo para recomeçar (os logins são mantidos).</p></div></div>`, `<div id="list-tecnicos" class="grid grid-cols-1 md:grid-cols-3 gap-3 p-4">${(db.tecnicos||[]).map(t=>`<div class="neo-card"><b>${escapeHtml(t.nome)}</b><p class="text-[12px] text-slate-500">${escapeHtml(t.especialidade||'Geral')}</p></div>`).join('')}</div>`);
  };
