...
(function(){
  ...
  window.cvAddItem=function(){if(!window.cvProduto) return toast('Selecione um produto','error'); const qtd=parseFloat(document.getElementById('cv-qtd').value)||1; if(window.cvProduto.categoria!=='Serviço' && window.cvProduto.categoria!=='Recarga' && (window.cvProduto.estoque||0)<=0) return (window.lfbAlert?window.lfbAlert('Produto sem estoque','Sem estoque') : alert('Produto sem estoque')); if(window.cvProduto.categoria!=='Serviço' && window.cvProduto.categoria!=='Recarga' && qtd>(window.cvProduto.estoque||0)) return (window.lfbAlert?window.lfbAlert('Estoque insuficiente. Disponível: '+(window.cvProduto.estoque||0),'Estoque insuficiente') : alert('Estoque insuficiente. Disponível: '+(window.cvProduto.estoque||0))); const preco=parseFloat(document.getElementById('cv-vunit').value)||window.cvProduto.preco||0; window.cvItens.push({produtoId:window.cvProduto.id,qtd,preco,subtotal:qtd*preco}); window.cvProduto=null; document.getElementById('cv-prod-search').value=''; document.getElementById('cv-vunit').value=''; cvRenderItens(); cvUpdateTotal();};
...
(function(){
  ...
  window.renderFinanceiro=function(){
    const sess=getSession(); if(!sess) return; const receber=db.contasReceber.filter(c=>c.empresaId===sess.empresaId); const totalRec=receber.reduce((s,c)=>s+(c.valor||0),0); const totalAberto=receber.filter(c=>(c.status||'aberto')!=='pago').reduce((s,c)=>s+(c.valor||0),0); const totalPago=receber.filter(c=>(c.status||'aberto')==='pago').reduce((s,c)=>s+(c.valor||0),0);
    document.getElementById('view-financeiro').innerHTML=neoPage('Financeiro', 'Contas a receber', `<button onclick="openModal('contaReceber')" class="neo-btn primary"><i class="ph ph-arrow-circle-down"></i>Receber</button>`, `<div class="grid grid-cols-1 md:grid-cols-3 gap-3 w-full"><div class="neo-card"><p class="neo-label">A receber</p><div class="neo-total !text-[24px]">${fmtMoney(totalRec)}</div></div><div class="neo-card"><p class="neo-label">Em aberto</p><div class="neo-total !text-[24px] !text-amber-600">${fmtMoney(totalAberto)}</div></div><div class="neo-card"><p class="neo-label">Já recebido</p><div class="neo-total !text-[24px] !text-emerald-600">${fmtMoney(totalPago)}</div></div></div>`, `<table class="neo-table"><thead><tr><th>Descrição</th><th>Cliente</th><th>Valor</th><th>Vencimento</th><th>Status</th></tr></thead><tbody>${receber.map(c=>{const cli=db.clientes.find(x=>x.id===c.clienteId)||{}; return `<tr><td>${escapeHtml(c.descricao||'')}</td><td>${escapeHtml(cli.nome||'')}</td><td><b>${fmtMoney(c.valor||0)}</b></td><td>${fmtDate(c.vencimento)}</td><td>${statusPill(c.status,c.status==='pago'?'ok':'wait')}</td></tr>`}).join('')||emptyRow(5,'Nenhum lançamento financeiro')}</tbody></table>`);
  };
  ...
  window.renderConfig=function(){
    document.getElementById('view-config').innerHTML=neoPage('Configurações', 'Empresa, técnicos e preferências do sistema', `<button onclick="exportBackup()" class="neo-btn"><i class="ph ph-download"></i>Exportar backup</button><button onclick="saveConfig()" class="neo-btn primary"><i class="ph ph-floppy-disk"></i>Salvar</button>`, `<div class="grid grid-cols-1 md:grid-cols-2 gap-3 w-full"><div class="neo-card"><label class="neo-label">Técnico</label><div class="flex gap-2"><input id="new-tecnico-nome" class="neo-input flex-1" placeholder="Nome do técnico"><button onclick="addTecnico()" class="neo-btn primary"><i class="ph ph-plus"></i>Cadastrar</button></div></div><div class="neo-card"><p class="neo-label">Backup</p><p class="text-[12px] text-slate-500 mt-1">Exporte um arquivo JSON de segurança dos seus dados.</p></div></div>`, `<div id="list-tecnicos" class="grid grid-cols-1 md:grid-cols-3 gap-3 p-4">${(db.tecnicos||[]).map(t=>`<div class="neo-card"><b>${escapeHtml(t.nome)}</b><p class="text-[12px] text-slate-500">${escapeHtml(t.especialidade||'Geral')}</p></div>`).join('')}</div>`);
  };
...
(function(){
  ...
  window.neoSalvarVenda = function(){
    ...
    if(gerarOS && venda){
      const numero = (window.proximoNumeroSimples ? window.proximoNumeroSimples('os', db.os, sess.empresaId) : String((db.os||[]).filter(o=>o.empresaId===sess.empresaId).length+1));
      db.os.push({
...
(function(){
  ...
  // ── FINANCEIRO (contas a RECEBER) com pesquisa, ordenação, filtro,
  //    seleção múltipla + excluir e histórico.
  //    v5.20.23: "contas a pagar" foi REMOVIDO do sistema (deletado, não
  //    ocultado) — junto com o filtro de tipo "Receber + Pagar".
  window.renderFinanceiro = function(){
    const sess=getSession(); if(!sess) return;
    ...
    const nomeCli=c=>{ const cli=db.clientes.find(x=>x.id===c.clienteId); return cli?cli.nome:(c.clienteNomeAntigo||''); };
    const receber=db.contasReceber.filter(c=>c.empresaId===sess.empresaId);
    const totalRec=receber.reduce((s,c)=>s+(c.valor||0),0);
    const totalAberto=receber.filter(c=>(c.status||'aberto')!=='pago').reduce((s,c)=>s+(c.valor||0),0);
    const totalPago=receber.filter(c=>(c.status||'aberto')==='pago').reduce((s,c)=>s+(c.valor||0),0);
    let all=receber.map(c=>({ref:c}));
    ...
    view.innerHTML=`<div class="neo-shell"><div class="neo-panel neo-float-in">
      <div class="neo-head"><div><h3>Financeiro</h3><p>Contas a receber — <b>duplo clique</b> (ou o olho 👁) abre o histórico do lançamento. Marque as caixas para excluir vários de uma vez.</p></div><div class="neo-actions"><button onclick="openModal('contaReceber')" class="neo-btn primary"><i class="ph ph-arrow-circle-down"></i>Receber</button><button id="btn-excluir-financeiro" onclick="excluirTitulosUnificado()" class="neo-btn danger"><i class="ph ph-trash"></i>Excluir</button></div></div>
      ...
      <div class="px-4 pb-3 flex flex-wrap items-center gap-2 border-b">
        <input id="neo-search-fin" value="${escapeHtml(document.getElementById('neo-search-fin')?.value||'')}" oninput="window.__finLim=150; renderFinanceiro()" class="neo-input flex-1 min-w-[220px]" placeholder="Pesquisar descrição, cliente, código...">
      ...
      <div class="overflow-auto max-h-[calc(100vh-380px)]"><table class="neo-table"><thead><tr><th class="w-[30px]"><input type="checkbox" title="Marcar todos" onclick="document.querySelectorAll('input[name=\\'fin-check-lote\\']').forEach(c=>c.checked=this.checked)"></th><th>Descrição</th><th>Cliente</th><th>Valor</th><th>Vencimento</th><th>Status</th><th></th></tr></thead><tbody>${mostrar.map(x=>{const c=x.ref; const cli=db.clientes.find(z=>z.id===c.clienteId); const nome=cli?cli.nome:(c.clienteNomeAntigo||''); return `<tr class="cursor-pointer" ondblclick="historicoLancamento('cr','${c.id}')"><td class="w-[30px]"><input type="checkbox" name="fin-check-lote" value="${c.id}" onclick="event.stopPropagation()"></td><td>${escapeHtml(c.descricao||'')} ${c.legadoCodigo?'<span class="text-[10px] text-slate-400">#'+escapeHtml(c.legadoCodigo)+'</span>':''}</td><td>${escapeHtml(nome)}${!cli&&c.clienteNomeAntigo?' <span class="text-[10px] text-slate-400">(sist. antigo)</span>':''}</td><td><b>${fmtMoney(c.valor||0)}</b></td><td>${fmtDate(c.vencimento)}</td><td>${statusPillFin(c.status)}</td><td><button onclick="event.stopPropagation(); historicoLancamento('cr','${c.id}')" class="neo-btn !px-2" title="Abrir histórico"><i class="ph ph-eye"></i></button></td></tr>`;}).join('')||'<tr><td colspan="7" class="text-center text-slate-500 py-12">Nenhum lançamento encontrado</td></tr>'}</tbody></table></div>
  ...
  // Histórico de um lançamento financeiro (conta a receber)
  // v5.20.23: contas a pagar foi removido do sistema.
  window.historicoLancamento = function(tipo,id){
    const arr=db.contasReceber;
    ...
    const nome=cli?cli.nome:(c.clienteNomeAntigo||'-');
    ...
    document.getElementById('modal-body').innerHTML=`<div class="space-y-3">
      <div class="neo-card"><p class="neo-label">Conta a receber</p><b class="text-[15px]">${escapeHtml(c.descricao||'')}</b><div class="flex flex-wrap gap-x-5 gap-y-1 mt-2 text-[13px]"><span>Valor: <b>${fmtMoney(c.valor||0)}</b></span><span>Vencimento: <b>${fmtDate(c.vencimento)}</b></span><span>Status: <b>${c.status||'aberto'}</b></span>${c.pagamentoData?'<span>Pago em: <b>'+fmtDate(c.pagamentoData)+'</b></span>':''}</div><p class="text-[12px] text-slate-500 mt-2">Cliente: <b>${escapeHtml(nome)}</b>${!cli&&c.clienteNomeAntigo?' (cadastro do sistema antigo)':''}${c.legadoCodigo?' • cód. antigo '+escapeHtml(c.legadoCodigo):''}</p></div>
      ${c.vendaId?`<button onclick="window.__origemFinanceiroVoltar={tipo:'${tipo}', id:'${id}'}; historicoVenda('${c.vendaId}')" class="neo-btn"><i class="ph ph-shopping-cart"></i>Abrir a venda de origem</button>`:''}
