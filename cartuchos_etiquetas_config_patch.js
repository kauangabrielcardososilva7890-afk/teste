...
function aplicarConfiguracoesCartuchos(dbRef, opts={}){
  ...
  cfg.etiquetas={
    ...
    margemSuperiorMm:5,
    margemEsquerdaMm:5,
    espacoHorizontalMm:1.5,
    espacoVerticalMm:1.2,
...
function htmlEtiquetas(codigos,opt={}){
  const c={colunas:ETQ_COLUNAS,linhas:ETQ_LINHAS,larguraMm:27,alturaMm:14,margemSuperiorMm:5,margemEsquerdaMm:5,espacoHorizontalMm:1.5,espacoVerticalMm:1.2,usarCodigoBarras:true,...(opt||{})};
...
function renderCardEtiquetas(){
  ...
  card.innerHTML=`<div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3"><div><h4 class="font-bold text-[15px]"><i class="ph ph-barcode"></i> Etiquetas de cartuchos</h4><p class="text-[12px] text-slate-500 mt-1">Gera etiquetas numéricas próprias, com código de barras, para colar nos cartuchos. Usa as etiquetas antigas só para sugerir o próximo número.</p></div><div class="text-[11px] text-slate-500">Próximo: <b class="text-slate-800">${et.proximoNumero||1}</b></div></div><div class="grid grid-cols-1 md:grid-cols-4 gap-3 mt-4"><div class="neo-card"><p class="neo-label">Etiquetas antigas</p><div class="neo-total">${ant.etiquetasEncontradas||0}</div></div><div class="neo-card"><p class="neo-label">Maior etiqueta</p><div class="neo-total">${ant.maiorNumeroEtiqueta||0}</div></div><div class="neo-card"><p class="neo-label">Produtos vazios removidos</p><div class="neo-total">${cfg.produtosCartuchoVazioRemovidos||0}</div></div><div class="neo-card"><p class="neo-label">Regra atual</p><div class="text-[13px] font-bold text-emerald-700 mt-2">vazio não é produto</div></div></div><div class="mt-4 grid grid-cols-1 md:grid-cols-4 gap-3 items-end"><label class="text-[12px] font-bold text-slate-600">Número inicial<input id="cart-etq-inicio" type="number" min="1" value="${et.proximoNumero||1}" oninput="cartEtiquetasAtualizarFim()" class="mt-1 w-full h-10 px-3 rounded-xl border"></label><label class="text-[12px] font-bold text-slate-600">Número final<input id="cart-etq-fim" type="number" min="1" value="${(et.proximoNumero||1)+ETQ_CAPACIDADE-1}" class="mt-1 w-full h-10 px-3 rounded-xl border"></label><button onclick="imprimirEtiquetasCartucho()" class="neo-btn primary"><i class="ph ph-printer"></i> Imprimir etiquetas</button><button onclick="atualizarConfigCartuchosAntigos()" class="neo-btn"><i class="ph ph-check-circle"></i> Atualizar</button></div><div class="mt-3 rounded-xl bg-amber-50 border border-amber-200 p-3 text-[12px] text-amber-900"><b>Importante:</b> as etiquetas novas são só números, sem prefixo e sem ano. Cartucho vazio não entra como produto separado no estoque.</div>`;
}
