...
ok(escola.indexOf('function esAbaAberta()') >= 0 && escola.indexOf('if(!esAbaAberta()) return;') >= 0, 'v6.0.5: automático SÓ trabalha com a aba do buscador aberta (decreto: consumia até fora dela)');
ok(escola.indexOf('try{ esAutoTique(); }catch(e){}') >= 0, 'v6.0.5: abrir a aba já confere dados velhos na hora');
...
ok(indexHtml.indexOf("DIGICOPY_APP_VERSION = '6.0.5'") >= 0, 'index.html carimbado (re-ancorado v6.0.5)');
ok(indexMob.indexOf("DIGICOPY_APP_VERSION = '6.0.5'") >= 0, 'mobile/www/index.html carimbada (re-ancorado v6.0.5)');
ok(pkg.version === '6.0.5', 'package.json carimbado (re-ancorado v6.0.5)');
