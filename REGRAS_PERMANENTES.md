...
## Trazidas da linha 5.21.x (fusão PR #27, 2026-09-20)

Importadas do `REGRAS.md` (apagado na fusão para não haver duas fontes). O que já existia acima com o mesmo sentido não foi repetido.

19. **Um patch por funcionalidade** (um arquivo por assunto, sem misturar).
20. **Antes de criar função nova, verificar se já existe**; se existir ou houver dúvida, perguntar — nunca duplicar.
21. **Remover = deletar de verdade** (nada de esconder nem flag 0/1 fingindo exclusão).
22. **Busca só no Enter ou na lupa** (nunca a cada tecla).
23. **Empresa única fixa** (`emp_digicopy`); `seedData` nunca apaga usuário real.
24. **Duplo clique abre o registro: consertar, nunca remover** (excluir/baixar/pagar/estornar/faturar/salvar nunca entram; testar na fiscal após fusões — fixar submenu usa cliques especiais).
25. **Testes quebrados/parados: consertar** (só deleta teste se ele testar coisa que nunca existiu no git).
26. **Chamados valem nos DOIS** (contrato e avulso), salvo ordem contrária.
27. **Não reabrir etiquetas nem vendas; áreas aceitas não mexer** sem pedido explícito.
28. **Toda atualização: link do site + PR + commit + resumo + manter o `RELATORIO_SESSAO.md`** (diário da sessão; entregas de teste seguem a regra 6; reenviar links sempre; NUNCA GitHack; publicar via PR).
29. Notas fundidas: `window.confirm` (era quebrado → corrigido no popup v2); banner rawgh removido (v5.21.5, aprovado); modo escuro oficial = `digi-escuro` (botão lua/sol descartado na fusão por duplicar).
