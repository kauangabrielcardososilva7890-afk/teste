...
ok('versão 5.22.57 no package.json e patch', P.VERSAO === '5.22.57' && /^5\.\d+\.\d+/.test(pkg.version));
...
ok('index carrega scripts na versão 5.22.x', /app\.bundle\.js\?v=5\.\d+\.\d+/.test(html) && JSON.parse(fs.readFileSync('bundle-manifest.json','utf8')).includes('ajustes_v52257_orcamento_sync_total_patch.js'));
ok('rodapé v5.22.x', /footer-version/.test(html) && /v5\.\d+\.\d+/.test(html));
ok('título v5.22.x', /Sistema Digicopy v5\.\d+\.\d+/.test(html));
