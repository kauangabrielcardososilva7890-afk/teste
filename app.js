...
function renderModalProduto(id){
  ...
  // Código automático: pega o maior código numérico existente e soma 1
  const codigos = (db.produtos||[]).filter(pr=>pr.empresaId===sess.empresaId).map(pr=>parseInt(String(pr.sku||'').replace(/\D/g,''))||0);
  const proximoCod = codigos.length ? Math.max(...codigos) + 1 : 1;
...
function renderFinanceiro(){
  ...
  const tipoCR=document.getElementById('filter-cr-tipo')?.value||'';
  const searchCR=(document.getElementById('search-cr')?.value||'').toLowerCase();
  let listCR=db.contasReceber.filter(c=>c.empresaId===sess.empresaId && (!statusCR||c.status===statusCR) && (!tipoCR||c.origem===tipoCR) && (!searchCR||c.descricao.toLowerCase().includes(searchCR)||(db.clientes.find(cl=>cl.id===c.clienteId)?.nome||'').toLowerCase().includes(searchCR)));
...
function updateBaixaMulti(){
  const checks = document.querySelectorAll('.cr-check:checked');
  const btn = document.getElementById('btn-baixa-multi');
  if(btn) btn.classList.toggle('hidden', checks.length < 2);
}
function toggleSelectAllCR(){
  const all = document.getElementById('cr-select-all');
  document.querySelectorAll('.cr-check').forEach(cb => cb.checked = all.checked);
  updateBaixaMulti();
}
function baixarMultiplasCR(){
  const checks = document.querySelectorAll('.cr-check:checked');
  if(checks.length < 2) return toast('Selecione pelo menos 2 títulos','error');
  if(!confirm('Dar baixa em ' + checks.length + ' títulos?')) return;
  const sess = getSession();
  let count = 0;
  checks.forEach(cb => {
    const crId = cb.dataset.id;
    const cr = db.contasReceber.find(c => c.id === crId && c.empresaId === sess.empresaId);
  ...
  if(count > 0){
    saveDB(); renderFinanceiro(); renderAuditoria();
    toast(count + ' títulos baixados','success');
  }
}
