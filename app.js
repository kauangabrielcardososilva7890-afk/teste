...
function deleteCliente(id){const sess=getSession(); const c=db.clientes.find(x=>x.id===id && x.empresaId===sess.empresaId); if(!c) return; const P=window.AJUSTES_V52023_PURE; const V=(P&&P.vinculosDoCliente)?P.vinculosDoCliente(db,id):[]; const fim=()=>{ let resumo=''; if(P&&P.removerClientesEmCascata){ const fora=P.removerClientesEmCascata(db,[id]); resumo=P.resumoCascata?P.resumoCascata(fora):''; } else { db.clientes=db.clientes.filter(x=>!(x.id===id && x.empresaId===sess.empresaId)); } logAction('cliente','excluir',id,`Excluído cliente ${c.nome||''}`+(resumo?` (com histórico: ${resumo})`:'')); saveDB(); renderClientes(); if(typeof renderFinanceiro==='function') renderFinanceiro(); renderAuditoria(); toast('Cliente excluído'+(resumo?' • também apagado: '+resumo:''),'success');}; if(typeof window.confirmSistema!=='function') return fim(); if(!V.length) return window.confirmSistema('Excluir o cliente "'+(c.nome||'')+'"? Apaga de vez (não é ocultar).','Excluir Cliente').then(ok=>{ if(ok) fim(); }); const aviso=(P&&P.textoHistorico)?P.textoHistorico([{cliente:c,vinculos:V}]):('Esse cliente tem histórico: '+V.join(', ')+'. Se apagar, o histórico vai sumir junto. Deseja continuar?'); window.confirmSistema(aviso,'Excluir Cliente').then(ok=>{ if(!ok) return; window.confirmSistema('Tem certeza que deseja fazer isso?\n\nO cliente e todo o histórico dele serão apagados de vez. Não dá para desfazer.','Confirmar exclusão').then(certeza=>{ if(certeza) fim(); }); });}
...
function renderModalProduto(id){
  const sess=getSession(); const isEdit=!!id; const p=isEdit?db.produtos.find(x=>x.id===id && x.empresaId===sess.empresaId):null;
  const codigos = (db.produtos||[]).filter(pr=>pr.empresaId===sess.empresaId).map(pr=>parseInt(String(pr.sku||'').replace(/\D/g,''))||0);
  db._seq = db._seq || {}; db._seq.produto = db._seq.produto || {};
  (function renumeraSeNecessario(){
    const prods = (db.produtos||[]).filter(x=>x.empresaId===sess.empresaId);
    let max = 0; prods.forEach(x=>{const n=parseInt(String(x.sku||'').replace(/\D/g,''))||0; if(n>max) max=n;});
    if(max > prods.length + 50){
      const ord=[...prods].sort((a,b)=> new Date(a.criadoEm||0)-new Date(b.criadoEm||0));
      ord.forEach((x,i)=>{x.sku=String(i+1); x.codigo=String(i+1);});
      max=ord.length;
      try{saveDB();}catch(e){}
    }
    const key=sess.empresaId||'global';
    if(db._seq.produto[key]==null) { db._seq.produto[key]=max+1; try{saveDB();}catch(e){} }
  })();
...
function saveProduto(){
  const _isEditProduto = !!document.getElementById('modal-title')?.innerText?.includes('Alterar');
  if(!_isEditProduto){
    const _seqKey = (typeof getSession==='function'?getSession():sess)?.empresaId || sess.empresaId || 'global';
    db._seq = db._seq||{}; db._seq.produto=db._seq.produto||{};
    const _peek = db._seq.produto[_seqKey]||1;
    const _inputVal = document.getElementById('f-prd-sku')?.value?.trim();
    if(_inputVal==String(_peek)){
      db._seq.produto[_seqKey]=_peek+1;
      try{saveDB();}catch(e){}
    }
  }
  const sess=getSession(); const id=window.modalContext?.id;
...
function renderFinanceiro(){
  ...
  const totalRecebidoMes=db.contasReceber.filter(cr=>cr.empresaId===sess.empresaId && cr.status==='pago' && cr.pagamentoData && new Date(cr.pagamentoData).getMonth()===new Date().getMonth()).reduce((s,c)=>s+c.valor,0);
  document.getElementById('fin-receber-mes').innerText=fmtMoney(totalReceberMes);
  const rEl=document.getElementById('fin-recebido-mes'); if(rEl) rEl.innerText=fmtMoney(totalRecebidoMes);
  document.getElementById('fin-saldo').innerText=fmtMoney(totalRecebidoMes);
  const statusCR=document.getElementById('filter-cr-status')?.value||'';
  const searchCR=(document.getElementById('search-cr')?.value||'').toLowerCase();
  let listCR=db.contasReceber.filter(c=>c.empresaId===sess.empresaId && (!statusCR||c.status===statusCR) && (!searchCR||c.descricao.toLowerCase().includes(searchCR)||(db.clientes.find(cl=>cl.id===c.clienteId)?.nome||'').toLowerCase().includes(searchCR)));
  ...
  const __cliFind=id=>db.clientes.find(c=>c.id===id);
  function origemLabel(cr){
    if(cr.vendaId){const v=db.vendas.find(x=>x.id===cr.vendaId); return v?'Venda '+v.numero:'Venda';}
    if(cr.leituraId) return 'Leitura';
    if(cr.contratoId){const ct=db.contratos.find(x=>x.id===cr.contratoId); return ct?'Contrato '+ct.numero:'Contrato';}
    return cr.origem||'Avulso';
  }
  function origemLink(cr){
    if(cr.vendaId) return "showVenda('"+cr.vendaId+"')";
    if(cr.leituraId) return "navigateTo('leituras')";
