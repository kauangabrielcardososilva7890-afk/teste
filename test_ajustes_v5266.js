...
ok('painel na 205, fila fecha com navegação fiscal firme + escuro íntegro v6.1.2', manifest[manifest.length - 20] === 'painel_gerente_patch.js' && manifest[manifest.length - 19] === 'fiscal_guard_patch.js' && manifest[manifest.length - 18] === 'nf_transmissao_patch.js' && manifest[manifest.length - 17] === 'autocura_empresa_central_nf_tela_patch.js' && manifest[manifest.length - 16] === 'perfis_nuvem_cura_sessao_patch.js' && manifest[manifest.length - 15] === 'permissoes_estorno_venda_patch.js' && manifest[manifest.length - 14] === 'fiscal_menu_completo_patch.js' && manifest[manifest.length - 13] === 'dashboard_inicio_clicavel_patch.js' && manifest[manifest.length - 12] === 'menus_fiscais_separados_patch.js' && manifest[manifest.length - 11] === 'permissoes_override_menus_fiscais_patch.js' && manifest[manifest.length - 10] === 'seis_submenus_velho_patch.js' && manifest[manifest.length - 9] === 'submenu_hover_nfe_patch.js' && manifest[manifest.length - 8] === 'navegacao_sem_tela_branca_patch.js' && manifest[manifest.length - 7] === 'ribbon_fiscal_estilo_antigo_patch.js' && manifest[manifest.length - 6] === 'fiscal_catalogo_completo_patch.js' && bundle.indexOf('PAINEL_GERENTE v5.26.6') >= 0);
...
ok('index.html carimbado', html.indexOf("DIGICOPY_APP_VERSION = '6.1.2'") >= 0 && html.indexOf('>v6.1.2<') >= 0);
