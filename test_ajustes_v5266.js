...
ok('painel na 205, fila fecha com navegação fiscal firme + escuro íntegro v6.1.3 (v7.0.24: +1 no fim, função única)', manifest[manifest.length - 25] === 'painel_gerente_patch.js' && manifest[manifest.length - 24] === 'fiscal_guard_patch.js' && manifest[manifest.length - 23] === 'nf_transmissao_patch.js' && manifest[manifest.length - 22] === 'autocura_empresa_central_nf_tela_patch.js' && manifest[manifest.length - 21] === 'perfis_nuvem_cura_sessao_patch.js' && manifest[manifest.length - 20] === 'permissoes_estorno_venda_patch.js' && manifest[manifest.length - 19] === 'fiscal_menu_completo_patch.js' && manifest[manifest.length - 18] === 'dashboard_inicio_clicavel_patch.js' && manifest[manifest.length - 17] === 'menus_fiscais_separados_patch.js' && manifest[manifest.length - 16] === 'permissoes_override_menus_fiscais_patch.js' && manifest[manifest.length - 15] === 'seis_submenus_velho_patch.js' && manifest[manifest.length - 14] === 'submenu_hover_nfe_patch.js' && manifest[manifest.length - 13] === 'navegacao_sem_tela_branca_patch.js' && manifest[manifest.length - 12] === 'ribbon_fiscal_estilo_antigo_patch.js' && manifest[manifest.length - 11] === 'fiscal_catalogo_completo_patch.js' && bundle.indexOf('PAINEL_GERENTE v5.26.6') >= 0);
...
ok('worker 5.28.0 (re-ancorado)', fs.readFileSync('cloudflare-worker/src/index.js', 'utf8').indexOf("WORKER_VERSION = '5.28.0'") >= 0);
