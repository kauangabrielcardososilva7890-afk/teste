...
ok('painel na 205, fila fecha com as 6 telas fiscais completas do catálogo v6.0.14', manifest[manifest.length - 15] === 'painel_gerente_patch.js' && manifest[manifest.length - 14] === 'fiscal_guard_patch.js' && manifest[manifest.length - 13] === 'nf_transmissao_patch.js' && manifest[manifest.length - 12] === 'autocura_empresa_central_nf_tela_patch.js' && manifest[manifest.length - 11] === 'perfis_nuvem_cura_sessao_patch.js' && manifest[manifest.length - 10] === 'permissoes_estorno_venda_patch.js' && manifest[manifest.length - 9] === 'fiscal_menu_completo_patch.js' && manifest[manifest.length - 8] === 'dashboard_inicio_clicavel_patch.js' && manifest[manifest.length - 7] === 'menus_fiscais_separados_patch.js' && manifest[manifest.length - 6] === 'permissoes_override_menus_fiscais_patch.js' && manifest[manifest.length - 5] === 'seis_submenus_velho_patch.js' && manifest[manifest.length - 4] === 'submenu_hover_nfe_patch.js' && manifest[manifest.length - 3] === 'navegacao_sem_tela_branca_patch.js' && manifest[manifest.length - 2] === 'ribbon_fiscal_estilo_antigo_patch.js' && manifest[manifest.length - 1] === 'fiscal_catalogo_completo_patch.js' && bundle.indexOf('PAINEL_GERENTE v5.26.6') >= 0);
...
ok('package.json na 6.0.9', pkg.version === '6.0.14');
ok('index.html carimbado', html.indexOf("DIGICOPY_APP_VERSION = '6.0.14'") >= 0 && html.indexOf('>v6.0.14<') >= 0);
