...
ok('painel na 205, fila fecha com o Menu Fiscal oficial v6.1.0', manifest[manifest.length - 17] === 'painel_gerente_patch.js' && manifest[manifest.length - 16] === 'fiscal_guard_patch.js' && manifest[manifest.length - 15] === 'nf_transmissao_patch.js' && manifest[manifest.length - 14] === 'autocura_empresa_central_nf_tela_patch.js' && manifest[manifest.length - 13] === 'perfis_nuvem_cura_sessao_patch.js' && manifest[manifest.length - 12] === 'permissoes_estorno_venda_patch.js' && manifest[manifest.length - 11] === 'fiscal_menu_completo_patch.js' && manifest[manifest.length - 10] === 'dashboard_inicio_clicavel_patch.js' && manifest[manifest.length - 9] === 'menus_fiscais_separados_patch.js' && manifest[manifest.length - 8] === 'permissoes_override_menus_fiscais_patch.js' && manifest[manifest.length - 7] === 'seis_submenus_velho_patch.js' && manifest[manifest.length - 6] === 'submenu_hover_nfe_patch.js' && manifest[manifest.length - 5] === 'navegacao_sem_tela_branca_patch.js' && manifest[manifest.length - 4] === 'ribbon_fiscal_estilo_antigo_patch.js' && manifest[manifest.length - 3] === 'fiscal_catalogo_completo_patch.js' && bundle.indexOf('PAINEL_GERENTE v5.26.6') >= 0);
...
ok('package.json na 6.0.9', pkg.version === '6.1.1');
ok('index.html carimbado', html.indexOf("DIGICOPY_APP_VERSION = '6.1.1'") >= 0 && html.indexOf('>v6.1.1<') >= 0);
