...
async function handleDeleteDevice(request, env) {
  // v5.24.32 — pedido dele: 'excluir os lixo antigo'. Seguro por desenho:
  ...
  // v5.24.32 relaxado a pedido dele (2ª cobrança: 'continua a mesma coisa,
...
let ultimoErroUso = '';
// v5.24.32 — sininho de atualização nova (pedido dele): 1 linha com a versão
...
async function garantirTabelaAppVersao(env){
  ...
  // v5.24.32 — SITE PRÓPRIO de atualizações (pedido dele): histórico de TODAS
  ...
  // v5.24.32 — PORTAL DE ATUALIZAÇÕES dele: ativa/desativa link, oculta,
...
// v5.24.32 — ECONOMIA DO MEDIDOR: medir a cota não pode GASTAR cota.
...
async function usoHoje(env){
  try{
    await garantirTabelaUso(env);
    // v5.24.32 — ASSINATURA PAGA CONFIRMADA POR ELE (2026-09-14, Workers Paid
...
// v5.24.32 — publicação "viva" = ativa, não oculta e não vencida nas opções
...
function linkDownload(origin, versao){ return origin + '/dl/' + encodeURIComponent(versao) + '.exe'; }
  ...
  if (request.method === 'GET' && url.pathname === '/v1/app-releases') {
    // v5.24.32 — histórico em JSON também (pras telas do sistema, se quiser).
  ...
  if (request.method === 'GET' && url.pathname === '/v1/app-release') {
    // v5.24.32 — leitura pública do sininho de atualização (versão + link + notas).
    await garantirTabelaAppVersao(env);
  ...
  if (request.method === 'POST' && url.pathname === '/v1/app-release') {
    // v5.24.32 — O PORTAL É SÓ DELE: todo gerenciamento exige aparelho ADMIN.
    ...
    if (!/^\d+(\.\d+)+$/.test(versao)) throw new ApiError(400, 'VERSAO_INVALIDA', 'Versão precisa estar no formato 5.24.32.');
  ...
  if (request.method === 'POST' && url.pathname === '/v1/release-file') {
    // v5.24.32 — o .exe em si sobe aqui (corpo = arquivo cru) e dorme no R2.
  ...
  if (request.method === 'GET' && url.pathname.startsWith('/dl/')) {
    // v5.24.32 — DOWNLOAD direto do .exe, porta da própria nuvem.
  ...
  if (request.method === 'GET' && url.pathname === '/atualizacoes') {
    // v5.24.32 — SITE DE DOWNLOAD (porta pública): NÃO é vitrine de histórico.
