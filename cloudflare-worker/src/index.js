...
let ultimoErroUso = '';
// v5.24.25 — sininho de atualização nova (pedido dele): 1 linha com a versão
...
async function garantirTabelaAppVersao(env){
  ...
  // v5.24.25 — SITE PRÓPRIO de atualizações (pedido dele): histórico de TODAS
...
// v5.24.25 — ECONOMIA DO MEDIDOR: medir a cota não pode GASTAR cota.
...
async function usoHoje(env){
  try{
    await garantirTabelaUso(env);
    // v5.24.25 — ASSINATURA PAGA CONFIRMADA POR ELE (2026-09-14, Workers Paid
...
async function route(request, env, ctx) {
  ...
  if (request.method === 'GET' && url.pathname === '/v1/app-releases') {
    // v5.24.25 — histórico em JSON também (pras telas do sistema, se quiser).
  ...
  if (request.method === 'GET' && url.pathname === '/v1/app-release') {
    // v5.24.25 — leitura pública do sininho de atualização (versão + link + notas).
    await garantirTabelaAppVersao(env);
  ...
  if (request.method === 'POST' && url.pathname === '/v1/app-release') {
    ...
    if (!/^\d+(\.\d+)+$/.test(versao)) throw new ApiError(400, 'VERSAO_INVALIDA', 'Versão precisa estar no formato 5.24.25.');
    ...
    // v5.24.25 — além de virar a atual (sininho), entra no histórico do site.
  ...
  if (request.method === 'GET' && url.pathname === '/atualizacoes') {
    // v5.24.25 — SITE PRÓPRIO (pedido dele): página pública com o histórico
