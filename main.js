...
app.whenReady().then(() => {
  ...
  registerBackupIPC();
  registerErroTxtIPC();
  // v5.24.28 — P6: contrato RTF abre DIRETO no Word (pedido dele/no.html):
...
function registerNfeCertIPC(){
  ...
  ipcMain.handle('nfe:cert-validade', async (_evt, payload) => {
    // v5.24.28 — conferir a validade do certificado SEM assinar nada: pede a
...
function registerPrinterMonitorIPC(){
  // v5.24.28 — Fase 1 do MONITOR DE IMPRESSORAS (pedido dele, 'ue faz'):
...
// ──────────────────────────────────────────────
// ERRO.TXT IPC (v5.24.28) — pedido dele: erro indevido vira linha num
