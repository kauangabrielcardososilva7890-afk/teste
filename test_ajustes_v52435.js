...
(async function(){
  ...
  ok(man[man.length-5] === 'ajustes_v52435_impressora_remanejo_final_patch.js' && man[man.length-3] === 'ajustes_v5250_leitura_overhaul_patch.js' && man[man.length-2] === 'ajustes_v5260_cnpj_gerente_patch.js' && man[man.length-1] === 'ajustes_v5262_login_nuvem_primeiro_patch.js', 'manifest: remanejo quinto a partir do fim; revisão antepenúltima; CNPJ+gerente penúltimo; login da nuvem primeiro v5.26.2 fecha a fila (vence sempre)');
  ...
  ok(fs.readFileSync('index.html', 'utf8').includes("DIGICOPY_APP_VERSION = '5.26.3'"), 'index 5.26.3');
  ok(fs.readFileSync('index.html', 'utf8').includes('>v5.26.3<'), 'rodapé v5.26.3');
  ok(fs.readFileSync('mobile/www/index.html', 'utf8').includes("DIGICOPY_APP_VERSION = '5.26.3'"), 'celular 5.26.3');
  ok(JSON.parse(fs.readFileSync('package.json', 'utf8')).version === '5.26.3', 'package.json 5.26.3');
