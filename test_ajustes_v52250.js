...
ok('versão 5.22.50 base', P.VERSAO === '5.22.50' && /^[56]\.\d+\.\d+/.test(pkg.version));
...
ok('index carrega os scripts', /app\.bundle\.js\?v=[56]\.\d+\.\d+/.test(html) && JSON.parse(fs.readFileSync('bundle-manifest.json','utf8')).includes('ajustes_v52250_exe_bundle_patch.js'));
ok('rodapé versão', /footer-version/.test(html) && /v[56]\.\d+\.\d+/.test(html));
