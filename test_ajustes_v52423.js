// test_ajustes_v52423.js — v5.24.23: pedido dele "função de publicar atualizações".
...
ok(cmpVersaoMaior('5.24.23','5.24.22')===true, 'cmp: 5.24.23 > 5.24.22');
ok(cmpVersaoMaior('5.25.0','5.24.99')===true, 'cmp: 5.25.0 > 5.24.99 (não trava no 9)');
ok(cmpVersaoMaior('5.24.23','5.24.23')===false, 'cmp: igual não avisa de novo');
ok(cmpVersaoMaior('5.24.0','5.24.23')===false, 'cmp: publique velha = silêncio');
...
ok(fs.readFileSync('index.html', 'utf8').includes("DIGICOPY_APP_VERSION = '5.24.23'"), 'index: versão 5.24.23');
ok(fs.readFileSync('index.html', 'utf8').includes('>v5.24.23<'), 'index: rodapé v5.24.23');
ok(fs.readFileSync('package.json', 'utf8').includes('"version": "5.24.23"'), 'package.json 5.24.23');
ok(wk.includes("'5.24.23'"), 'worker carimbado 5.24.23');

if (falhas > 0) { console.error(`\n${falhas} assert(s) FALHARAM`); process.exit(1); }
console.log('\nTudo OK — v5.24.23 (sininho de atualização + publicador na config).');
