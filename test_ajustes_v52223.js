...
ok('versão 5.22.23+', (/^[56]\.\d+\.\d+$/.test(pkg.version) && (parseInt(pkg.version.split('.')[0],10)>=6 || parseInt(pkg.version.split('.')[1],10)>=23 || parseInt(pkg.version.split('.')[2],10)>=23)) && html.includes('app.bundle.js?v='+pkg.version));
