...
## Fusão PR #27 — linha 5.21.x incorporada (2026-09-20)

Linha `arena/01a0bb58-teste` (v5.21.3–v5.21.6) fundida aqui (merge da `01a0590a` + trabalho 5.21.x, via PR #27). Detalhe completo viveu no `RELATORIO_SESSAO.md` daquela branch (preservado no histórico git).

**Incorporado da 5.21.x:**
- Fiscal/módulos dinâmicos no padrão neo (`modulos_neo_visual_patch.js`, 223º): Enter/lupa, ordenar ▲▼, duplo clique detalhe, Mostrar mais 50/50, rodapé Imprimir, `filtrarComIndice` (sem trava em tabela grande).
- Duplo clique global (`duplo_clique_patch.js`, 224º + `abrirLogAuditoria` no app.js): abre cadastro/detalhe nas grades; blocklist protege excluir/pagar/etc. Pós-fusão: testar na fiscal (fixar submenu).
- Dono gerencia nuvem: `systemAdmin()` aceita `dono` além de `admin` (mantido; teste `Dono também gerencia a nuvem` passa).
- CI: `.github/workflows/ci.yml` (check + test + worker) — veio do main junto.
- Docs: `REGRAS.md` fundido em `REGRAS_PERMANENTES.md` (regras 19–29); automações em `docs/`.

**Removido antes/durante (com aprovação):** lua/sol (duplicava o `digi-escuro` oficial); banner rawgh; testes de patches inexistentes; asserção `!systemAdmin()&&token()` (superada pelo papel-do-aparelho v6.0.6).
