...
ok('filtrarComIndice existe', typeof P.filtrarComIndice === 'function');
ok('filtrarComIndice mantém índice original', (()=>{ const r = P.filtrarComIndice(dados, ['COD', 'NOME'], 'a', ''); return r.length === 2 && r[0].i === 0 && r[1].i === 1 && r[1].row.NOME === 'abacaxi'; })());
ok('filtrarComIndice sem busca devolve tudo com índice', (()=>{ const r = P.filtrarComIndice(dados, ['COD'], '', ''); return r.length === 3 && r[2].i === 2; })());
ok('sem indexOf na lista toda (não trava)', !/dados\.indexOf/.test(code));
ok('detalhe tem rodapé com Imprimir', /data-mig-print/.test(code) && /imprimirRegistroMigrado/.test(code));
ok('rodapé padrão p-4 border-t', /p-4 border-t/.test(code));
