...
ok('versão 5.22.53 base', P.VERSAO === '5.22.53' && /^[56]\.\d+\.\d+/.test(pkg.version));
...
ok('index carrega os scripts na versão 5.22', /app\.bundle\.js\?v=[56]\.\d+\.\d+/.test(html) && JSON.parse(fs.readFileSync('bundle-manifest.json','utf8')).includes('ajustes_v52253_login_tela_branca_patch.js'));
ok('rodapé na versão 5.22', /footer-version/.test(html) && /v[56]\.\d+\.\d+/.test(html));
