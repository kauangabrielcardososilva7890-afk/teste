// Teste v5.24.30 — "abrir já mostrando o que eu escolhi" + 4.2 sem fantasma:
...
// v5.24.30 SUPERSEDEU o auto-abrir o 1º: a ficha agora desenha a LISTA do
...
ok(patch.indexOf('clitabRenderSoSelecionados(sub, ids)') >= 0, 'v5.24.30: a LISTA mostra os marcados (o 1º não abre mais sozinho)');
...
ok(worker.indexOf("const WORKER_VERSION = '5.24.30'") >= 0, 'worker carimba v5.24.30');
ok(bundle === bundleM, 'bundles raiz e mobile idênticos');
ok(bundle.indexOf('clitabAbrirDireto') >= 0, 'abridor presente no bundle');
ok(indexHtml.indexOf("DIGICOPY_APP_VERSION = '5.24.30'") >= 0 && indexHtml.indexOf('app.bundle.js?v=5.24.30') >= 0, 'index.html na v5.24.30');
ok(indexMob.indexOf("DIGICOPY_APP_VERSION = '5.24.30'") >= 0, 'mobile/www/index.html na v5.24.30');
ok(pkg.version === '5.24.30', 'package.json v5.24.30');
if(falhas){ console.error('\n' + falhas + ' FALHA(S) v5.24.30'); process.exit(1); }
console.log('\nTudo certo v5.24.30!');
