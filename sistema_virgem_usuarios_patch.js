...
function aplicarVirgemSePreciso(){
  // v4.9.69: NUNCA limpar dados automaticamente em atualização.
...
function renderLoginVirgem(){
  ...
  box.innerHTML=`<div class="mb-6 flex items-center gap-3 p-3 rounded-xl bg-[#e8eaf8] border border-[#c9ceef]"><div class="w-10 h-10 rounded-xl bg-[#0a1e8a] text-white grid place-items-center font-bold text-[14px]">DG</div><div><p class="font-bold text-[13px] leading-tight">Sistema Digicopy</p><p class="text-[11px] text-slate-600">Sistema virgem • login por usuário/senha</p></div></div><div class="mb-6"><h2 class="text-[20px] font-bold tracking-tight">Login do usuário</h2><p class="text-[13px] text-slate-500 mt-1">Sem CNPJ. Use seu usuário e senha.</p></div><form class="space-y-4" onsubmit="event.preventDefault(); doLoginUser();"><div><label class="text-[11px] font-bold uppercase text-slate-500">Usuário</label><input id="login-user" autocomplete="username" value="" class="mt-1.5 w-full h-[48px] px-4 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-[#0a1e8a] focus:ring-4 focus:ring-[#0a1e8a]/10 outline-none text-[14px]"></div><div><label class="text-[11px] font-bold uppercase text-slate-500">Senha</label><input id="login-senha-user" type="password" autocomplete="current-password" class="mt-1.5 w-full h-[48px] px-4 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-[#0a1e8a] focus:ring-4 focus:ring-[#0a1e8a]/10 outline-none text-[14px]"></div><button type="submit" class="w-full h-[48px] rounded-xl bg-[#0a1e8a] text-white font-semibold text-[14px] hover:bg-[#08176e] transition shadow-lg shadow-[#0a1e8a]/20">Entrar</button><p class="text-[11px] text-slate-400 text-center">Digite seu usuário e senha para entrar.</p></form>`;
}
...
console.log('[DIGICOPY] sistema_virgem_usuarios_patch.js v4.9.69 carregado');
})();
